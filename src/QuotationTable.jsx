...
function QuotationTable({ data, clearDataItems, deleteByIndex, }) {
  ...
  const totalAmount = data.reduce((acc, v) => acc + v.qty * v.ppu, 0);
  const totalDiscount = data.reduce((acc, v) => acc + (typeof v.discount === 'number' ? v.discount : 0), 0);
  ...
  return (
    <Container>
      ...
      <Table striped bordered hover>
        ...
        <tbody>{
          data.map((v, i) => {
            let amount = v.ppu * v.qty - (typeof v.discount === 'number' ? v.discount : 0);
            const calculationString = `${v.ppu} x ${v.qty} - ${v.discount} = ${amount}`;
            return (
              <tr key={i}>
                ...
                <td className={style.textRight}>{calculationString}</td> 
